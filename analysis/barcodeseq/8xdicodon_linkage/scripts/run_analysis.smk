...
def get_fastq_file_for_sample_name(wildcards):
  ...
  """
  sample_id = sample_annotations.loc[sample_annotations['sample_name'] == wildcards.sample_id, 'sample_id'].item()
...
rule subset_to_annotated_inserts:
  ...
  input:
    ...
    insert_annotations = '../annotations/dicodon_yeast.csv',
  output:
  ...
  shell:
    ...
    awk -F, -v OFS=, '
      ...
      {{
        if ($1 in inserts) {{
          print barcode_num, inserts[$1], $2, $3;
          barcode_num++
        }}
      }}
      ' \
